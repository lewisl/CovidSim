...
# new series should track new positives, not net values
...
const infect_risk_by_age = [.3, .4, .5, .6, .7]
...
"""
mapcond2tran = (unexposed=-1, infectious=-1, recovered=1, dead=6, nil=2, mild=3, sick=4, severe=5)
...
function spread!(locale; dat=openmx)
    ...
    push!(bugq, (day=ctr[:day], locale=locale, spreaders = sum(spreaders), contacted = sum(numcontacted), 
                    touched = sum(numtouched), unexposed=grab(unexposed, agegrps, lag, locale, dat=dat), infected=newinfected))
...
=#
    ...
    #=
        ...
        - use the accessibility factor in a binomial sample:  was the contact "successful" in causing infection?
    ...
    if !isapprox(sum(s_a_pop_pct), 1.0, atol=1e-8)  
...
function how_many_infected(numtouched, all_unexposed)
    # transmissibility by agegrp of recipient

    newinfected = zeros(Int,size(numtouched,1))  # agegrps
    for i in eachindex(numtouched) # this probabilisticly determines if contact resulted in contracting the virus
        newsick = binomial_one_sample(numtouched[i], infect_risk_by_age[i])
        newsick = newsick < all_unexposed[i] ? newsick : floor(Int,.8 * all_unexposed[i])  # TODO use .8 * all_unexposed?
        newinfected[i] = newsick
    end
