...
function seed_case_gen(day, cnt, lag, cond, agegrp) # these args go into the returned seed! case
    function scase(locale; opendat, isodat, env)  # args must match runcases loop in run_a_sim
...
function isolate_case_1(locale; opendat, isodat, env)
    if ctr[:day] == 15
...
function unisolate_case_1(locale; opendat, isodat, env)
    if ctr[:day]  == 120
...
function isolate_case_2(locale; opendat, isodat, env)
    if ctr[:day] == 15
...
function unisolate_case_2(locale; opendat, isodat, env)
    if ctr[:day]  == 69
        unisolate!(1.0,[unexposed,nil],agegrps,1,locale; opendat=opendat, isodat=isodat)
        unisolate!(1.0,[mild,sick, severe],agegrps,1:laglim,locale; opendat=opendat, isodat=isodat)
    end
...
function unisolate_case_2b(locale; opendat, isodat, env)
    if ctr[:day]  == 84
...
function isolate_case_3(locale; opendat, isodat, env)
    if ctr[:day] == 40
        isolate!(.40,[unexposed, nil],agegrps,1,locale; opendat=opendat, isodat=isodat)
        isolate!(.75,[mild,sick, severe],agegrps,1:laglim,locale; opendat=opendat, isodat=isodat)
    elseif ctr[:day] == 50
...
function unisolate_case_3(locale; opendat, isodat, env)
    if ctr[:day]  == 80
        unisolate!(1.0,[unexposed,nil],agegrps,1,locale; opendat=opendat, isodat=isodat)
        unisolate!(1.0,[mild,sick,severe],agegrps,1:laglim,locale; opendat=opendat, isodat=isodat)
    end
